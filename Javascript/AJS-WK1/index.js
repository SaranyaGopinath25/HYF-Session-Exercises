...
function renderMentorCard(mentor) {
    console.log("3333333333::::  "+mentor.name);
...
function forEachHomemade(array, functionToExecute) {
  // FOREACH TODO: implement forEach
    console.log("222222");
      array.forEach(functionToExecute);
...
function renderCards(mentorsToShow) {
    console.log("4444444444 ::::::  "+mentorsToShow.length);
  const container = document.getElementById("result");
  container.innerHTML = "";
...
//   const firstMentor = mentorsToShow[0];
mentorsToShow.forEach(function(mentor){
    console.log("5555555555::::  "+mentor.name);
    wrapper.appendChild(renderMentorCard(mentor));
});
const firstMentor = mentorsToShow[0]
  wrapper.appendChild(renderMentorCard(firstMentor));
...
function showAllMentors() {
  ...
  document.getElementById("resultLabel").textContent =
    ...
    forEachHomemade(mentors,renderMentorCard);
...
//   const mentorNames = [];
const mentorNames = mentors.map((mentor) => {
   return mentor.name;
})
...
mentors.forEach((mentor) => {
    if(mentor.yearOfExperience > 7){
        return experiencedMentors.push(mentor);
    }
})
...
const mentorsThatStartWithA = [];
mentors.forEach(mentor => {
    if(mentor.name.startsWith("A")){
        return mentorsThatStartWithA.push(mentor)
    }
})
...
/* 
NOTES : CLEAN CODE
1 - Keep simple separate functions for each functionality
2 - Don't overload a single function
3 - Don't alter the global variable
*/
